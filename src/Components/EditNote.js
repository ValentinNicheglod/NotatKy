...
import { Badge, Chip, IconButton, Modal } from '@material-ui/core'
...
const EditNote = ({
    ...
    const tagModal = (
        <div className="modal-col">
            <h2 id="simple-modal-title">{largeWidth ? "Añadir etiquetas" : "Editar etiquetas"}</h2>
            <hr/>
            <div className="tag-cont">
                {tags.length > 0
                        ...
                        <div className="actions-notes p-0 px-3 row">
                                {largeWidth &&
                                    <div className="col-md-9 m-0 d-flex align-items-center">
                                        {note.tags && note.tags.map((tag, index) => index < 5 && (
                                            <div key={index} className="tab-indicator mx-1 px-2" style={{backgroundColor: tag.color}}>
                                        ...
                                        {note.tags && note.tags.length > 5 &&
                                            <Badge 
                                                className="mx-2"
                                                badgeContent={`+${note.tags.length - 5}`} color="primary"
                                            >
                                            <LocalOfferIcon />
                                        </Badge>
                                        }
                                ...
                                <div className={largeWidth ? "col-md-3 m-0 d-flex justify-content-end align-items-center" : "m-0 d-flex justify-content-between align-items-center"}>
                                    {!largeWidth &&
                                        <div className="d-flex align-items-center tag-cont-show">
                                            {note.tags && note.tags.map((tag, index) => index < 2 && (
                                            <div key={index} className="tab-indicator mx-1 px-1" style={{backgroundColor: tag.color}}>
                                            ...
                                            {note.tags && note.tags.length > 2
                                                ? <Badge 
                                                    className="mx-2"
                                                    badgeContent={note.tags && note.tags.length > 2 && `+${note.tags.length - 2}`} color="primary"
                                                >
                                                    <IconButton
                                                        className="btn p-0"
                                                        title="Añadir etiqueta"
                                        ...
                                        {largeWidth &&
                                            <b title="Fecha de creación" style={{cursor: 'default'}}>
                                                <small>{note.createdAt && moment(note.createdAt.slice(0, 10), "YYYY-MM-DD").format("DD/MM/YY")}</small>
                                            </b>
                                        }
                                    </div>
                                </div>
                        ...
                        {allNotes && allNotes.length === 0
                            ...
                            : <>
                                <img 
                                    ...
                                    draggable={false}
                                />
                                <b className="select-text">Selecciona una nota para ver su contenido o editarla</b>
                            </>
                        }
